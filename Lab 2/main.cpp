...
void print()
{
	...
	screen += 63;
	outp(0x20, 0x0A);					// (port, value) - write value to port; 0x0A - line feed
...
void interrupt  newint08(...) { print(); oldint8(); }		// change interuption
...
void initialize()
{
	oldint8 = getvect(0x08);	// get interuption vector
	...
	_disable(); // CLI - Command Line Interface

	// interrupt initializtion for Master			ICW - Initialization Command Words before work
...
int main()
{
	...
	FP_OFF(fp) = 0x2c;		// data segment shift
	_dos_freemem(*fp);		// to free it

	_dos_keep(0, (_DS - _CS) + (_SP / 16) + 1);		//the termination code - 1st parameter, the amount of memory that should be reserved for the program after its completion - 2nd parameter
