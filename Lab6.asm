...
START:
    ...
    mov es,ax                          
    lea dx,MSG_ENTER					;сообщение о вводе времени нахождения в PM
    mov ah,9h
    int 21h
    ...
    mov ah, 9h
    int 21h
    lea dx,MSG_HELLO			;вывод сообщения о приветствии
    mov ah,9h
    int 21h
    mov ah,7h
    int 21h         
                ;подготовка часов реального времени
PREPARE_RTC:                                    
    mov al,0Bh
    out 70h,al                                  
    in  al,71h                                  
...
WRITE_CODE_PM:                                  
    ...
    mov ax,CODE_PM					;bx = команды защищенного режима
    xor dh,dh
    mov dl,ah
    shl ax,4
    shr dx,4
...
FILL_IDT:                           
    ...
    lea eax, TIMER_HANDLER		;заполнение прерывания от таймера                      
    mov IDT_TIMER.OFFS_L,ax		;заносим прерывание в таблицу
    shr eax, 16
    mov IDT_TIMER.OFFS_H,ax

    lea eax, KEYBOARD_HANDLER   ;заполнение прерывания от клавиатуры            
    ...
    or  al,00000001b                ;включаем защищенный режим            
    mov cr0,eax                     ;загружаем результат и переходим в PM
...
PREPARE_TO_RETURN:                              
    ...
    push eax                                    
    push edi
                    ;переинициализируем контроллеры для PM
...
ENABLE_INTERRUPTS_0:                            
	...
	nop
    sti
                    ;переводим архитектуру на IA-32	
GO_TO_CODE_PM:                                  
    db 66h  		;директива для перехода                                    
    retf

                ;возврат в RM(реальный режим)
...
ENABLE_INTERRUPTS:                              
    ...
    nop
    sti
                ;установить ограничения вентиля А20	
DISABLE_A20:                                    
    in  al,92h
    and al,11111101b                            
    out 92h, al
...
EXIT:                                           
    mov ax,3h
    int 10H                                         
    lea dx,MSG_EXIT				;сообщение о выходе
    mov ah,9h
    int 21h                                     

    call FILL_CR_0_BUFFER_RM    ;заполнение регистра CR0              
...
;выполняемый код защищенного режима
...
DATA_BEGIN      = $
    ...
    GDT_CODE_RM S_DESC <SIZE_CODE_RM-1,,,10011010b,0,>         ;селектор 16, сегмент команд реального режима
    ...
    GDT_CODE_PM S_DESC <SIZE_CODE_PM-1,,,10011010b,01000000b,> ;селектор 48, сегмент команд реального режима   
